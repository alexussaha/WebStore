...
using WebStore.Domian.DTO.Identity;
...
namespace WebStore.Clients.Identity
{
    public class UsersClient : BaseClient, IUserClient
    {
        ...
        {
            _Logger.LogInformation("Изменение имени пользвоаетля {0} на {1}",
                user.UserName,
                name);
            user.UserName = name;
        ...
        {
            await PostAsync(
                $"{_ServiceAddress}/RemoveClaims", new RemoveClaimDTO { User = user, Claims = claims },
                cancel);
        }
